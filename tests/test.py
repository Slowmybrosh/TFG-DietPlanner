...
class TestBuscador(unittest.TestCase):
    ...
    def test_buscar_ingrediente_id(self):
        assert self.buscador.buscarIngrediente_ID(0) == [(0, 'bacalao')]

    def test_buscar_ingrediente_nombre(self):
        assert self.buscador.buscarIngrediente_Nombre('bacalao') == [(0, 'bacalao')]

    def test_buscar_receta_id(self):
        resultado = self.buscador.buscarRecetas_ID(1)
    ...
    def test_buscar_receta_nombre(self):
        resultado = self.buscador.buscarRecetas_Nombre('bacalao')
    ...
    def test_buscar_receta_ingredentes(self):
        resultado = self.buscador.buscarRecetas_ingredientes([0,1,2,3,4,5,6,7])
